...
void bezierCubic(const Point2d *P0, const Point2d *P1, const Point2d *P2, const Point2d *P3, 
  ...
  for(double t = 0.0; t < 1.0; t+=step){
    double B0 = pow(1-t, 3);
    double B1 = (3.0*t)*pow((1-t), 2);
    double B2 = 3.0*pow(t, 2)*(1.0-t);
    double B3 = pow(t, 3);

    newPoint.x = (B0 * P0->x) + (B1 * P1->x) + (B2 * P2->x) + (B3 * P3->x);
    newPoint.y = (B0 * P0->y) + (B1 * P1->y) + (B2 * P2->y) + (B3 * P3->y);
    vecPushBack(trajectory_points, &newPoint);
  }
}
